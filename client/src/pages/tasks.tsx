...
import { Plus, Pencil, Trash2, ListChecks, CalendarDays, Cpu, CircuitBoard } from "lucide-react";
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight" data-testid="text-page-title">Attivit&agrave;</h1>
          </div>
          <p className="text-xs text-muted-foreground mt-1 font-mono tracking-wider uppercase">Task Allocation Module</p>
        </div>
        <Dialog open={open} onOpenChange={(o) => { if (!o) closeDialog(); else setOpen(true); }}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="font-mono tracking-wide">{editTask ? "// MODIFICA TASK" : "// NUOVO TASK"}</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField control={form.control} name="plannedHours" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs font-mono tracking-wider uppercase">Ore Pianificate</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.5"
                ...
                <div>
                  <Label className="text-xs font-mono tracking-wider uppercase mb-2 block">Assegna Operatori</Label>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {users.length === 0 && <p className="text-[10px] text-muted-foreground font-mono">No operators available</p>}
                  </div>
                </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-muted-foreground text-xs font-mono tracking-wider">NO TASKS ALLOCATED</p>
            <p className="text-[10px] text-muted-foreground/60 mt-1 font-mono">Click "Nuova Attivit&agrave;" to create</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={task.id} className="hover-elevate" data-testid={`card-task-${task.id}`}>
                <CardHeader className="flex flex-row items-start justify-between gap-1 space-y-0 pb-2">
                  <div className="min-w-0 space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded">{String(idx + 1).padStart(2, "0")}</span>
                      <CardTitle className="text-base truncate">{task.name}</CardTitle>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      {project && <Badge variant="secondary" className="font-mono text-[10px] tracking-wider">{project.name}</Badge>}
                      {task.plannedHours && (
                        <span className="text-[11px] text-primary font-mono font-semibold">{task.plannedHours}h</span>
                      )}
                    </div>
                    {(task.startDate || task.endDate) && (
                      <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground font-mono">
                        ...
                        {task.startDate && task.endDate && <span className="text-primary/40">&gt;</span>}
                    ...
                    {assigned.length > 0 && (
                      <div className="flex gap-1.5 flex-wrap">
                        {assigned.map(u => u && (
                          <Badge key={u.id} variant="outline" className="text-[10px] font-mono">{u.name} {u.surname}</Badge>
                        ))}
                      </div>
                    )}
                  </div>
